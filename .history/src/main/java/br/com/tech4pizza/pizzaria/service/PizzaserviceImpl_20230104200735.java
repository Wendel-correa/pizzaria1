...
public class PizzaserviceImpl implements PizzaService{
    ...
    public PizzaCompletoDto cadastrarPizza(PizzaCompletoDto pizza) {
      ...
      repositorio.save(pizzaBanco);
        return new ModelMapper().map(pizzaBanco, PizzaCompletoDto.class);
    }
    @Override
  public Optional<PizzaCompletoDto> atualizarPizzaPorId (String id, PizzaCompletoDto pizza) {
    Optional<Pizza> pizzaBanco = repositorio.findById(id);

    if (pizzaBanco.isPresent()) {
      pizzaBanco.get().setId(id);
repositorio.save(pizzaBanco.get());
